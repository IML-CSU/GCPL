            ...
            switch (party) {
                case "generator":
                    ...
                    if (useChkDci) {
                        ...
                        System.out.println("Gen. starts patientlinkage algorithm with filtring using chk DC for those matched by any filter ...");
                        ...
                        if (PassNumOfOnes) {
                            ...
                            for (int i = 0; i < numOfUsedBFs ; i++) {
                                ...
                                GenWssBFsWith1scntAndChkDCi<GCSignal> gen = new GenWssBFsWith1scntAndChkDCi<>(port, Mode.REAL, threads, BFData, numOf1s, ws_bin, threshold_bin,chkDCi, BRecsToProcess, PartyA_IDs);
                                gen.implement();
                                tres = gen.getLinkage();
                                ...
                                chkDCi= UpdateChkDCi(chkDCi,tres);
                    ...
                    if (useChkDci) {
                        ...
                        if (PassNumOfOnes) {
                            ...
                            for (int i = 0; i < numOfUsedBFs ; i++) {
                                BFData = BFHlp1.getData_bin(i);
                                numOf1s = BFHlp1.getBF1sCnt(i);
                                EnvWssBFsWith1scntAndChkDCi<GCSignal> env = new EnvWssBFsWith1scntAndChkDCi<>(addr, port, Mode.REAL, threads, BFData, numOf1s, ws_bin, threshold_bin,chkDCi, ARecsToProcess, PartyB_IDs);
                                env.implement();
                                tres = env.getLinkage();
                                //tres = BFHlp1.getLinkageOrgIndexes(tres, party);
