 ...
 * @author cf
 */
public class Main_BFpartsClr1s {
        ...
        if (useBFs) {
            ...
            switch (party) {
                case "generator":
                    ...
                    if (useChkDci) {
                        ...
                        if (PassNumOfOnes) {
                            ...
                            System.out.println(" patientlinkage With BF #1s passed on the clr...");
                            ...
                            for (int i = 0; i < numOfUsedBFs; i++) {
                                ...
                                for (int Part = 1; Part <= BFHlp1.getnumOfParts(); Part++) {
                                    ...
                                    GenWssBFsWithCLR1scntAndChkDCi<GCSignal> gen = new GenWssBFsWithCLR1scntAndChkDCi<>(port, Mode.REAL, threads, BFpartData, intNumOf1s, chkPartDCi, BRecsToProcess, PartyA_IDs);
                                    gen.implement();
                ...
                case "evaluator":
                    ...
                    if (useChkDci) {
                        System.out.println((new Date())+" :\n Eval. ((V2)) starts patientlinkage algorithm with: \n -Partitioned BFs \n-filtring using chk DC for those matched by any filter ...");
                        //System.out.println((new Date())+" :\n Eval. starts patientlinkage algorithm with filtring using chk DC for those matched by any filter ...");
                        t0 = System.currentTimeMillis();
                        if (PassNumOfOnes) {
                            boolean[][][] numOf1s;
                            numOf1s = help1.numOfOnesInBFs;
                            ...
                            for (int i = 0; i < numOfUsedBFs; i++) {
                                ...
                                for (int Part = 1; Part <= BFHlp1.getnumOfParts(); Part++) {
                                    ...
                                    EnvWssBFsWithCLR1scntAndChkDCi<GCSignal> env = new EnvWssBFsWithCLR1scntAndChkDCi<>(addr, port, Mode.REAL, threads, BFpartData, intNumOf1s, chkPartDCi, ARecsToProcess, PartyB_IDs);
                                    env.implement();
                ...
                for (int i = 0; i < AllBfsTotBFPartsRes.length; i++) {
                    ...
                    if (results_save_path != null) {
                        try (FileWriter writer = new FileWriter(results_save_path,true)) {
                            ...
                            Logger.getLogger(Main_BFpartsClr1s.class.getName()).log(Level.SEVERE, null, ex);
                        }
                    }
